...
(function () {
  ...
  function buildEmailBody(opts) {
    ...
    lines.push('');
    lines.push('— Sent from www.manuchaitanya.com');
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('change', function () {
      ...
      if (!$('ratingsStatus').classList.contains('text-success')) {
        setStatus('Draft saved for today.', 'info');
      }
    });
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (!selected) {
        setStatus('Please select a rating (1–5) for all 4 categories, then submit.', 'error');
        return;
      }
